    ...
    public function groupemailpost(Request $request)
    {
        switch ($request->type) {
            case 'user':
                ...
                break;
            case 'driver':
                $users = Driver::whereStatus(1)->where('email_verified',1)->get(['id','name','email']);
                break;
            case 'agent':
                $users = Agent::whereStatus(1)->where('email_verified',1)->get(['id','name','email']);
                break;
        ...
        return back()->with('success','Email sent to all.');
