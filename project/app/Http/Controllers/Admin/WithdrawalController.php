...
class WithdrawalController extends Controller
{
    ...
    {
        $withdrawlogs = Withdrawals::where('status', 1)->latest()->with(['method','user','currency','driver','agent'])->paginate(15);
        return view('admin.withdraw.withdraw_all',compact('withdrawlogs'));
    }
    public function pending()
    {
        $withdrawlogs = Withdrawals::where('status', 0)->latest()->with(['method','user','currency','driver','agent'])->paginate(15);
        return view('admin.withdraw.withdraw_all',compact('withdrawlogs'));
    }
    public function rejected()
    {
        $withdrawlogs = Withdrawals::where('status', 2)->latest()->with(['method','user','currency','driver','agent'])->paginate(15);
        return view('admin.withdraw.withdraw_all',compact('withdrawlogs'));
    }
    ...
    public function withdrawAccept(Withdrawals $withdraw)
    {
        ...
        elseif ($withdraw->driver_id){
            $user = $withdraw->driver;
         
            $trnx              = new Transaction();
            $trnx->trnx        = str_rand();
            $trnx->user_id     = $withdraw->driver_id;
            $trnx->user_type   = 2;
            $trnx->currency_id = $withdraw->currency->id;
            $trnx->amount      = $withdraw->amount;
            $trnx->charge      = $withdraw->charge;
    ...
    public function withdrawReject(Request $request, Withdrawals $withdraw)
    {
        ...
        if($withdraw->user_id){
            ...
            $user = $withdraw->driver;
            $wallet = Wallet::where('user_id',$withdraw->driver_id)->where('user_type',2)->where('currency_id',$withdraw->currency_id)->firstOrFail();
