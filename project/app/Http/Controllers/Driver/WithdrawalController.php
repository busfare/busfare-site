...
class WithdrawalController extends Controller
{
    ...
    {
        $wallets = Wallet::where('user_id',driver()->id)->where('user_type',2)->where('balance', '>', 0)->get();
        return view('driver.withdraw.withdraw_form',compact('wallets'));
    }
    ...
    public function history(Request $request)
    {
        ...
        $withdrawals = Withdrawals::when($search,function($q) use($search) {return $q->where('trx',$search);})->where('driver_id',driver()->id)->latest()->paginate(15);
        return view('driver.withdraw.history',compact('withdrawals','search'));
    }
}
