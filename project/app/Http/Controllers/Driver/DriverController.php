...
class DriverController extends Controller
{
    ...
    {
        $wallets            = Wallet::where('user_type',2)->where('user_id',driver()->id)->get();
        $recentWithdraw     = Withdrawals::where('driver_id',driver()->id)->take(7)->get();
        $recentTransactions = Transaction::where('user_id',driver()->id)->where('user_type',2)->take(7)->get();
        return view('driver.dashboard',compact('wallets','recentWithdraw','recentTransactions'));
    }

    public function generateQR()
    {
        return view('driver.qr');
    }

    public function apiKeyForm()
    {
        $cred = ApiCreds::whereDriverId(driver()->id)->first();
        if(!$cred){
        ...
        return view('driver.api_key_form',compact('cred'));
    }
    ...
    {
        $user = driver();
        return view('driver.profile_setting',compact('user'));
    }
    ...
    public function transactions()
    {
        ...
        $transactions = Transaction::where('user_id',driver()->id)->where('user_type',2)
        ...
        return view('driver.transactions',compact('transactions'));
    }
    ...
    public function trxDetails($id)
    {
        ...
        return view('driver.trx_details',compact('transaction'));
    }
    ...
    {
        return view('driver.twostep.two_step');
    }
    ...
    public function twoStepSendCode(Request $request)
    {
        ...
        if (Hash::check($request->password, $user->password)) {
            ...
            return redirect(route('driver.two.step.verify'))->with('success','OTP code is sent to your phone.');
        } else {
    ...
    {
        return view('driver.twostep.verify');
    }
    ...
    public function twoStepVerifySubmit(Request $request)
    {
        ...
        $user->save();
        return redirect(route('driver.two.step'))->with('success',$msg);
    }

    public function kycForm()
    {
        if(driver()->kyc_status == 2) return back()->with('error','You have already submitted the KYC data.');
        if(driver()->kyc_status == 1) return back()->with('error','Your KYC data is already verified.');
        $kycForm = KycForm::where('user_type',2)->get();
        return view('driver.kyc.kyc_form',compact('kycForm'));
    }
    ...
    public function kycFormSubmit(Request $request)
    {
        ...
        $user->kyc_status = 2;
        $user->save();
        return redirect(route('driver.dashboard'))->with('success','KYC data has been submitted for review.');
    }
    ...
    public function moduleOff($module)
    {
        ...
        if($mod &&  $mod->status == 1){
            return redirect(url('driver/'.$key));
        }
        return view('driver.module_off',compact('module'));
    }
