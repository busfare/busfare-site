...
class PaymentApiController extends Controller
{
    ...
    {
        $payment = DriverPayment::where('payment_id',request('payment_id'))->where('status','!=',1)->firstOrFail();
        session()->put('payment',encrypt($payment));
        return view('driver_payment.auth',compact('payment'));
    }
    ...
    public function livePayment($payment,$charge)
    {
        ...
        $driverWallet = Wallet::where('user_type',2)->where('user_id',$driver->id)->where('currency_id',$payment->currency_id)->first();
        ...
        $trnx->charge      = 0;
        $trnx->remark      = 'driver_api_payment';
        $trnx->type        = '-';
        $trnx->details     = trans('Payment to driver : ').  $driver->email;
        $trnx->save();
        ...
        $receiverTrnx->user_id     = $driver->id;
        ...
        $receiverTrnx->type        = '+';
        $receiverTrnx->remark      = 'driver_api_payment';
    ...
    public function checkValidity(Request $request)
    {
        ...
        $payment = DriverPayment::where('payment_id',$request->payment_id)->first();
