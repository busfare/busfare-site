        ...
        <div class="card">
                ...
                <form action="" class="row justify-content-md-end flex-grow-1">
                  <div class="col-12 col-md-6 col-xl-3 mb-2">
                    <select  class="form-control w-100" onChange="window.location.href=this.value">
                        ...
                        <option value="{{filter('remark','driver_payment')}}" {{request('remark')=='driver_payment' ? 'selected':''}}>@lang('Driver Payment')</option>
                        
                        <option value="{{filter('remark','driver_api_payment')}}" {{request('remark')=='driver_api_payment' ? 'selected':''}}>@lang('Driver API Payment')</option>
            ...
            <div class="card-body text-center">
                ...
                @if ($transactions->hasPages())
                  {{ $transactions->links('driver.partials.paginate') }}
                @endif
            </div> 
...
@push('script')
    <script>
      ...
      $('.details').on('click',function () { 
        var url = "{{url('driver/transaction/details/')}}"+'/'+$(this).data('data').id
