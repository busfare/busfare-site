                  ...
                  @csrf
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <div class="form-label">@lang('Driver Email') <span class="ms-2 check"></span></div>
...
@push('script')
    ...
    <script>
        ...
        $('.receiver').on('focusout',function () { 
            var url   = '{{ route('user.check.driver') }}';
            var value = $(this).val();
            ...
            $.post(url,data,function(res) {
                ...
                if(res['data'] != null){
                    ...
                    $('.check').text(`@lang('Valid driver found.')`).addClass('text-success');
                    $('.transfer').attr('disabled',false)
                } else {
                    if($('.check').hasClass('text-success')){
                        $('.check').removeClass('text-success');
                    }
                    $('.check').text('@lang('Driver not found.')').addClass('text-danger');
